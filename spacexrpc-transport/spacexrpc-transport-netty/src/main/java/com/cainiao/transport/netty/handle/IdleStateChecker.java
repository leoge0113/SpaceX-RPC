package com.cainiao.transport.netty.handle;

import com.cainiao.common.util.SystemClock;
...
public class IdleStateChecker extends ChannelDuplexHandler {
    ...
    private final ChannelFutureListener writeListener = (ChannelFutureListener) (future) -> {
        firstAllIdleEvent = firstWriterIdleEvent
                = true;
    ...
    private void initialize(ChannelHandlerContext ctx) {
        ...
        if (readerIdleTimeMillis > 0) {
            readerIdleTimeout = timer.newTimeout(new ReaderIdleTimeoutTask(ctx),
                    readerIdleTimeMillis, TimeUnit.MILLISECONDS);

        }
        if (writerIdleTimeMillis > 0) {
            writerIdleTimeout = timer.newTimeout(new WriterIdleTimeoutTask(ctx),
    ...
    private final class ReaderIdleTimeoutTask implements TimerTask {
        ...
        public void run(Timeout timeout) throws Exception {
            if (!ctx.channel().isOpen() || timeout.isCancelled()) {
                return;
            }
            ...
            if (nextDelay <= 0) {//已经超时
                readerIdleTimeout = timer.newTimeout(this, readerIdleTimeMillis, TimeUnit.MILLISECONDS);//ne-time execution
                try {
