...
import com.cainiao.transport.Directory;
...
public class DirectoryJChannelGroup {
    ...
    public CopyOnWriteGroupList find(Directory directory) {
        System.out.println("[DirectoryJChannelGroup][find]directory=" + directory.toString());
        String _directory = directory.directory();
        System.out.println("[DirectoryJChannelGroup][find]_directory=" + _directory);
     ...
     * 获取指定group的引用计数
     */
    public int getRefCount(SChannelGroup group) {
     ...
     * 指定group的引用计数 +1
     */
    public int incrementRefCount(SChannelGroup group) {
     ...
     */
    public int decrementRefCount(SChannelGroup group) {
        AtomicInteger counter = groupRefCounter.get(group);
        if (counter == null) {
            return 0;
        }
    ...
    static class GroupRefCounterMap extends ConcurrentHashMap<SChannelGroup, AtomicInteger> {

        private static final long serialVersionUID = 6590976614405397299L;

        public AtomicInteger getOrCreate(SChannelGroup key) {
